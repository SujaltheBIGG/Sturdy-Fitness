...
**Spec:** issue #172 + comentario ciclo-compra (https://github.com/CLIENT-ORG/sturdy-app/issues/172#issuecomment-4888183183).
...
- [ ] **Step 2:** Run: `pnpm --filter @sturdy/core test` → Expected: FAIL (módulo no existe).
...
- [ ] **Step 4:** Run: `pnpm --filter @sturdy/core test` → Expected: PASS.
- [ ] **Step 5: Commit**
...
- [ ] **Step 4:** Run tests → PASS. Correr la suite completa: `pnpm --filter @sturdy/core test` → todo verde (156 previos + nuevos).
- [ ] **Step 5: Commit**
...
- [ ] **Step 3:** `pnpm --filter @sturdy/core test` → PASS.
- [ ] **Step 4: Commit**
...
- [ ] **Step 4:** `pnpm --filter @sturdy/core test` → PASS (los hooks no tienen tests: I/O; la lógica testeable ya está en `shopping.ts`).
- [ ] **Step 5: Commit**
...
- **Imports de core**: replicar EXACTO el estilo de import que usa `PantryTable.tsx` / `PantryPlanSection.tsx` (alias `@sturdy/core` vs subpaths). Lo de arriba es orientativo.
...
```tsx
import { useAuthUser } from '@/lib/use-auth-user'
import { usePantryItems } from '@sturdy/core/hooks/usePantry'   // estilo de import: replicar el de pantry.tsx
...
```bash
pnpm --filter @sturdy/core test          # 156 previos + ~25 nuevos, todo verde
